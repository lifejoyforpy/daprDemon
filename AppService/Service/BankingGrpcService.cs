    ...
    public class BankingGrpcService : AppCallback.AppCallbackBase
    {
        ...
        public override async Task<InvokeResponse> OnInvoke(InvokeRequest request, ServerCallContext context)
        {
            ...
            switch (request.Method)
            {
                ...
                case "deposit":
                case "withdraw":
                    var transaction = request.Data.Unpack<GrpcService.Transaction>();
        ...
        public override async Task<TopicEventResponse> OnTopicEvent(TopicEventRequest request, ServerCallContext context)
        {
            if (request.PubsubName == "pubsub")
            {
                ...
                var transaction = new GrpcService.Transaction() { Id = input.Id, Amount = (int)input.Amount, };
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public async Task<GrpcService.Account> GetAccount(GetAccountRequest input, ServerCallContext context)
        {
            var state = await _daprClient.GetStateEntryAsync<Models.Account>(StoreName, input.Id);
            return new GrpcService.Account() { Id = state.Value.Id, Balance = (int)state.Value.Balance, }; 
        }
